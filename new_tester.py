...
class Tester:
    ...
    def pass_find_pass(self):
        ...
        df = inv_idx.find('Ðåêòîð ÌÃÓ', encoding=None)
    ...
    def test_find(self):
        ...
        inv_idx.get_inverted_index()
        df = inv_idx.find('Ðåêòîð ÌÃÓ', encoding=None)
        assert all(word in inv_idx.preprocess(row['message']) for idx, row in df.iterrows() for word in inv_idx.preprocess('Ðåêòîð ÌÃÓ'))
